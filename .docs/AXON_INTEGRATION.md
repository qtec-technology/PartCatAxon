...
1 source line = 1 draft Item + 1 draft Term
...
### AI Auto-Recommend Scope (3 features)

ระบบ AI ที่ต้องพัฒนาสำหรับ PartCatalog (ใช้ GPT + Gemini):

| Feature | Primary | Fallback | Output Fields |
|---|---|---|---|
| Weight & Dimension Lookup | Grainger DB search | AI estimate from web | `itemWeightKg`, `L×W×H`, `chargeableWeightKg` |
| HS Code & Duty Suggestion | Historical HS Code data (Worakamon's team) | AI inference | `hsCode`, `dutyPercent` |
| Import Permit & Shelf Life | Product category rules | AI check | `permitRequired`, `shelfLifeRequired` |

### Three-Tier Fallback (ทุก field)

```
Tier 1 → AXON extracted from supplier document
Tier 2 → Kim's AI (GPT/Gemini) auto-recommend  
