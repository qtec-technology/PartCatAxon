...
const formatUpdatedDate = (value: string): string => {
  ...
  return isValid(parsed) ? format(parsed, 'dd-MMM-yyyy HH:mm:ss') : text;
};

const ensureOption = (list: TermSalesPersonOption[], code: string, name: string): TermSalesPersonOption[] => {
  const normalizedCode = String(code || '').trim();
  if (!normalizedCode || normalizedCode === '0') return list;
  const fallbackName = String(name || '').trim() || `Unknown sales person (code ${normalizedCode})`;
  return list.some((row) => row.code === normalizedCode)
    ? list
    : [{ code: normalizedCode, name: fallbackName, active: 'N' }, ...list];
};
    ...
    <div className="flex flex-col gap-4 h-full min-h-0">
      ...
      <div className="border border-term-red rounded-md overflow-hidden shadow-sm">
        ...
        <div className="p-3 space-y-2 bg-white">
          ...
          <CalcRow label="Duty Tax (DT)(FR)(THB)" value={fmt(calcResults.DT_FR)} />
      ...
      <div className="border border-term-red rounded-md overflow-hidden shadow-sm">
        ...
        <div className="px-3 py-2 flex items-center justify-between">
          ...
          <span className="text-xs text-gray-500">
            {formatUpdatedDate(formData.updatedDate)}
          </span>
        </div>
      </div>
    </div>
