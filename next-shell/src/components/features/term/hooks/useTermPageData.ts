...
export function useTermPageData({
    ...
    useEffect(() => {
        ...
        const fetchLookups = async () => {
            ...
            try {
                const [data, critical, salesSubLocationRows] = await Promise.all([
    ...
    useEffect(() => {
        ...
        let cancelled = false;
        const controller = new AbortController();
        const selectedLocation = String(formData.purchaseTermLocation || '').trim();

        const loadPurchaseSubLocations = async () => {
            if (!selectedLocation) {
                setPurchaseSubLocations([]);
                setFormData((prev) => (
                    prev.purchaseSubLocation
                        ? { ...prev, purchaseSubLocation: '' }
                        : prev
                ));
                return;
            ...
            try {
                const rows = await lookupApi.getSubLocations('AP', selectedLocation, {
                    signal: controller.signal,
                });
                if (cancelled) return;

                const nextOptions = uniqueStrings(rows.map((row) => String(row.name || '').trim()));
                setPurchaseSubLocations(nextOptions);
                setFormData((prev) => {
                    if (!prev.purchaseSubLocation || nextOptions.includes(prev.purchaseSubLocation)) return prev;
                    return { ...prev, purchaseSubLocation: '' };
                });
            } catch (error) {
                if (isAbortError(error)) return;
                clientLogger.error('Failed to load purchase sub locations', error);
                if (!cancelled) {
                    setPurchaseSubLocations([]);
                }
            }
        ...
        void loadPurchaseSubLocations();

        return () => {
            cancelled = true;
            controller.abort();
        };
    }, [formData.purchaseTermLocation, mode, readOnlyMode]);
    ...
    useEffect(() => {
        ...
        setFormData((prev) => {
            let next = prev;
            const resolvedSalesPersonName = resolveSalesPersonNameFromLookup(
                prev.salesPerson,
                prev.salesPersonName,
                salesPersons,
            );
            if (resolvedSalesPersonName && prev.salesPersonName !== resolvedSalesPersonName) {
                next = next === prev
                    ? { ...prev, salesPersonName: resolvedSalesPersonName }
                    : { ...next, salesPersonName: resolvedSalesPersonName };
            }

            const resolvedSourcedByName = resolveSalesPersonNameFromLookup(
                prev.sourcedBy,
                prev.sourcedByName,
                salesPersons,
            );
            if (resolvedSourcedByName && prev.sourcedByName !== resolvedSourcedByName) {
                next = next === prev
                    ? { ...prev, sourcedByName: resolvedSourcedByName }
                    : { ...next, sourcedByName: resolvedSourcedByName };
            }
