...
type TermPageMode = 'new' | 'view' | 'edit';

interface TermPageClientProps {
    initialMode: TermPageMode;
    termId?: string;
    sourceItemId?: string;
}
...
function mapFormDataToApiPayload(formData: TermFormData, itemId?: number): Record<string, unknown> {
    return {
        ...
        U_VendorBPA: formData.vendorBPA ? 'Y' : 'N',
...
export function TermPageClient({
    initialMode,
    termId,
    sourceItemId,
}: TermPageClientProps) {
    ...
    const [mode, setMode] = useState<TermPageMode>(initialMode);
    ...
    const currentTermId = parseNullableInt(termId);
    ...
    const handleSave = useCallback(async () => {
        ...
        try {
            ...
            } else if (effectiveMode === 'edit' && termId) {
                const payload = mapFormDataToApiPayload(formData);
                await termApi.updateTerm(termId, payload);
                await reloadRecord();
    ...
    }, [effectiveMode, formData, isSaving, navigate, readOnlyMode, reloadRecord, sourceItemId, termId]);

    const handleExit = () => {
        navigate('/partcatalog');
    };

    const handleBack = () => navigate('/partcatalog');
    const handleEdit = () => {
        setMode('edit');
    };
    const handleCancel = () => {
    ...
    const handleDelete = useCallback(async () => {
        ...
        const firstConfirmed = window.confirm(
            `Delete term "${parsedTermId}"? This action cannot be undone.`
        );
        ...
        try {
            setIsDeleting(true);
            await termApi.deleteTerm(parsedTermId, secondConfirmText);
    ...
    const handleSendRfq = async () => {
        ...
        try {
            const result = await termApi.getVendorEmail(termId);
            window.location.href = result.mailtoUrl!;
        } catch (err: unknown) {
            const message = err instanceof Error ? err.message : 'Failed to get vendor email';
            toast.error(message);
        } finally {
